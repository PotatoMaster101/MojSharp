...
public class ProfileResponseTest
{
    ...
    {
        // arrange, act, assert
        Assert.Throws<ArgumentNullException>(() => new ProfileResponse("foo", null!, null!));
        Assert.Throws<ArgumentNullException>(() => new ProfileResponse("foo", new Player("foo", "bar"), null!));
        Assert.Throws<ArgumentNullException>(() => new ProfileResponse("foo", null!, Array.Empty<ProfileProperty>()));
    }
